...
f = open(filedatetime + "_" + str(dataname) + "_LinSVC_classic_round_log.txt", 'w')
...
for i in range(len(alldata)):
    ...
    roundtext = i+1
    #訓練模型名稱
    modelname = material.replace('/','').replace('*','')+"_LinSVC_classic_round_"+str(i)+".pickle"
    ...
    while testdata:
        ...
        for a in range(len(x)):
            ...
            pro.append(classifier.prob_classify_many(x[a]))
...
#寫入csv
with open(filedatetime + 'LinSVC_classic.csv', 'w', newline='') as csvfile:
