...
f = open(filedatetime + "_" + str(dataname) + "_ME_classic_round_log.txt", 'w')
...
for i in range(len(alldata)):
    ...
    roundtext = i+1
    #訓練模型名稱
    modelname = material.replace('/','').replace('*','')+"_ME_classic_round_"+str(i)+".pickle"
    ...
    #讀取模型
    #fm = open('result/bayes/sumen/2gram_classic/datasumen_NB_classic_round_8.pickle', 'rb')
    #classifier = pickle.load(fm)
    #fm.close()
...
#寫入csv
with open(filedatetime + 'ME_classic.csv', 'w', newline='') as csvfile:
